module Kaminari
  ...
  module ActionViewExtension
    ...
    # A helper that renders a next and previous link without performing a count on collection.
    #
    #   <%= paginate_without_count @articles %>
    #
    # ==== Options
    # * <tt>:window</tt> - The "inner window" size (4 by default).
    # * <tt>:outer_window</tt> - The "outer window" size (0 by default).
    ...
    def paginate_without_count(scope, options = {}, &block)
      ...
      paginator = Kaminari::Helpers::PaginatorWithoutCount.new(
        self, options.reverse_merge(
          current_page: scope.current_page,
          per_page: scope.limit_value,
          remote: false
    ...
    def link_to_previous_page(scope, name, options = {}, &block)
      prev_page = Kaminari::Helpers::PrevPage.new self, options.reverse_merge(
        current_page: scope.current_page
      )
    ...
    def link_to_next_page(scope, name, options = {}, &block)
      next_page = Kaminari::Helpers::NextPage.new self, options.reverse_merge(
        current_page: scope.current_page
      )
